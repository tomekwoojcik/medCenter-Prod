...
export function SearchProfession({ place }) {
    ...
    const { handleProfesion, uniqeProfessionMed } = UserAuth();

    return (
        <div className="search_Div">
            <label className="label_input">
                Wyszukaj specjalizacje:
                <input value={word} onChange={e => setWord(e.target.value)} on onClick={() => { setHidden(prev => prev = "block") }} placeholder={place} className="search" type="text"></input>
