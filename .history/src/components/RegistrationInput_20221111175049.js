            ...
            {filterArr.map((el) => {
                return (
                    <div key={el.id} className="input_registration">
                        ...
                        <p className="reg_end">Kończysz rejestrację do: {`${el.data.title}a ${el.data.medicalSpecialization}a ${el.data.name}  ${el.data.surname}`}</p>
                        <p className="visit_place">Miejsce wizyty: {`${el.data.adress}, ${el.data.city}`}</p>
                        <form>
                            <label>
                                Podaj datę wizyty:
