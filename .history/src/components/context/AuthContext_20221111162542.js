...
export const AuthContextProvider = ({ children }) => {
    ...
    const readyRegistration = userRegistration.filter((el) => {
        return el.data.dateOfTheVisit === string && el.data.hourOfTheVisit === string

    })
    console.log(readyRegistration);
    //-------------------------------------------------------------------
    return (
        <UserContext.Provider value={{ createUser, user, logout, signIn, states, handleButton, cities, handleCityButton, medPerson, handleProfesion, medHospital, lastNfzUniqeArr, filtrElements, uniqeArrCityHospital, handleHospitalInCityButton, filtrHospitalInCity, arrHospitalCity, uniqeProfessionMed, dataConnect, selectHospital, handleCity, handleRegistration, notificationNumber, nullToEndRegistration, person, book, setBook, sendData }}>
            {children}
