...
export const AuthContextProvider = ({ children }) => {
    ...
    let lastNfzUniqeArr = uniqeArr.filter(el => { return el !== "Łódzki OW NFZ" });

    const search = () => {
        let ar = lastNfzUniqeArr.map((el) => { return el.includes(wordOne) }).indexOf(true);
        const a = [];
        a.push(uniqeArr[ar]);
        return lastNfzUniqeArr = a;
    }
    ...
    //-------------------------------------------------------------------
    return (
        <UserContext.Provider value={{ createUser, user, logout, signIn, states, handleButton, cities, handleCityButton, medPerson, handleProfesion, medHospital, lastNfzUniqeArr, filtrElements, uniqeArrCityHospital, handleHospitalInCityButton, filtrHospitalInCity, arrHospitalCity, uniqeProfessionMed, dataConnect, selectHospital, handleCity, handleRegistration, notificationNumber, nullToEndRegistration, person, book, setBook, sendData, readyRegistration, setHidden, hidden, hiddenSec, setHiddenSec, hiddenThr, setHiddenThr, wordOne, setWordOne, search }}>
            {children}
