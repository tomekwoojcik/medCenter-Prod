...
export const AuthContextProvider = ({ children }) => {
    ...
    const getMedHospital = () => {
        const medCollection = collection(db, `HospitalCol`);
        getDocs(medCollection)
            .then(response => {
                const meds = response.docs.map(doc => ({ data: doc.data(), id: doc.id }))
                setMedHospital(meds)
    ...
    const arrStateNFZ = [];
    medHospital.map((el) => {
        arrStateNFZ.push(el.data.nazwaOddziałuNFZ);
    })
    ...
    // const filtrData = () => {
    //     setFiltrElements(() => {
    //         medHospital.sort((el) => { return el.data.nazwaOddziałuNFZ === checkCity })
    //         console.log(filtrElements)
    //     })
    // }
    let arr = []
    const filtrData = (checkCity) => {
        setFiltrElements(arr = medHospital.filter(el => { return el.data.nazwaOddziałuNFZ.include(checkCity) }))
    }
    console.log(arr);
    ...
    return (
        <UserContext.Provider value={{ createUser, user, logout, signIn, states, handleButton, cities, handleCityButton, dataMed, medPerson, handleProfesion, medHospital, uniqeArrStateNFZ }}>
            {children}
