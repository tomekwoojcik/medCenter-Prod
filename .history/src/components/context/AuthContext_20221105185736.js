...
export const AuthContextProvider = ({ children }) => {
    ...
    const getMed = () => {
        const medCollection = collection(db, "PediatraCol");
        getDocs(medCollection)
            .then(response => {
                const meds = response.docs.map(doc => ({ data: doc.data(), id: doc.id }))
                setMedPerson(meds)
    ...
    console.log(handleCityButton);

    return (
        <UserContext.Provider value={{ createUser, user, logout, signIn, states, handleButton, cities, handleCityButton, dataMed, medPerson, handleProfesion }}>
            {children}
