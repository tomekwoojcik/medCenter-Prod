...
export const AuthContextProvider = ({ children }) => {
    ...
    const arr = userRegistration.filter((el) => {
        return el.data.dateOfTheVisit === null && el.data.hourOfTheVisit === null

    })
    useEffect(() => {
        setNotificationNumber(arr)
    })
    // const nullToEndRegistration = 

    // useEffect(() => { setNotificationNumber(nullToEndRegistration.length) }, [])


    // let number = nullToEndRegistration.length;
    console.log(notificationNumber)

    //-------------------------------------------------------------------------------------------
    return (
        <UserContext.Provider value={{ createUser, user, logout, signIn, states, handleButton, cities, handleCityButton, medPerson, handleProfesion, medHospital, lastNfzUniqeArr, filtrElements, uniqeArrCityHospital, handleHospitalInCityButton, filtrHospitalInCity, arrHospitalCity, uniqeProfessionMed, dataConnect, selectHospital, handleCity, handleRegistration, notificationNumber }}>
            {children}
