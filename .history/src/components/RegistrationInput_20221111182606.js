            ...
            {filterArr.map((el) => {
                return (
                    <div key={el.id} className="input_registration">
                        ...
                        <p className="reg_end">Miejsce wizyty: <span className="text_decor">{`${el.data.adress}, ${el.data.city}`}</span></p>
                        <form className="input_board">
                            <label>
                                Podaj datę wizyty:
                            ...
                            <Button className="button_input" onClick={() => { sendData(book, inputDate, inputTime) }}>Wyślij rezerwację</Button>
                        </form>
                    </div>)
            }
            )}
            <div className="image_input"></div>
